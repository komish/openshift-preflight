...
import (
	...
	"github.com/redhat-openshift-ecosystem/openshift-preflight/internal/x/formatters"
...
func NewCommand(
	...
	for i, p := range plugin.RegisteredPlugins() {
		...
		plcmd.RunE = func(cmd *cobra.Command, args []string) error {
			return run(ctx, args, invocation, config, &runtime.ResultWriterFile{})
		}
...
func run(
	...
	resultWriter plugin.ResultWriter,
) error {
	...
	textResults, err := formatters.GenericJSONFormatter(ctx, results)
	if err != nil {
